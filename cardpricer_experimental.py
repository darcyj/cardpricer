...
print('  some may take longer than others.')
print('  hint: basic lands = slow.')
results = []
nq = len(qrys_binned)
for i in range(nq):
...
for cr in cheapo_results:
  if cr.get('name').lower() != cr.get('matchname').lower():
...
# check for catastrophic sorting error (no idea what would have caused this)
